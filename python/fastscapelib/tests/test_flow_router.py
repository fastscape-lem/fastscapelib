...
from fastscapelib.flow_graph import FlowGraph, FlowRouterMethods, DummyFlowRouter, MultipleFlowRouter, SingleFlowRouter
...
class TestSingleFlowRouter():
    ...
    def setup_class(cls):
        ...
        cls.result_profile_elevation = cls.profile_flow_graph.update_routes(cls.profile_elevation)

        raster_grid = RasterGrid([4, 4], [1.1, 1.2], RasterBoundaryStatus(NodeStatus.FIXED_VALUE_BOUNDARY), [])
        cls.raster_flow_graph = FlowGraph(raster_grid, SingleFlowRouter())
        cls.raster_elevation = np.array([[0.82,  0.16,  0.14,  0.20],
                                         [0.71,  0.97,  0.41,  0.09],
                                         [0.49,  0.01,  0.19,  0.38],
                                         [0.29,  0.82,  0.09,  0.88]])
        cls.result_raster_elevation = cls.raster_flow_graph.update_routes(cls.raster_elevation)
    ...
    def test_receivers(self):
        npt.assert_equal(self.profile_flow_graph.receivers()[:, 0], np.r_[1, 2, 2, 2, 3, 6, 7, 7])

        npt.assert_equal(self.raster_flow_graph.receivers()[:, 0], np.r_[1,  2,  7,  7,
                                                                         9,  9,  7,  7,
                                                                         9,  9,  9,  7,
                                                                         9,  9,  9,  14])
    ...
    def test_receivers_distance(self):
        npt.assert_equal(self.profile_flow_graph.receivers_distance()[:, 0], np.r_[1., 1., 0., 1., 1., 1., 1., 0.]*2.2)

        dia = np.sqrt(1.1**2 + 1.2**2)
        npt.assert_equal(self.raster_flow_graph.receivers_distance()[:, 0], np.r_[1.2,  1.2,  dia,  1.1,
                                                                                  dia,  1.1,  1.2,  0.0,
                                                                                  1.2,  0.0,  1.2,  1.1,
                                                                                  dia,  1.1,  dia,  1.2 ])
    ...
    def test_receivers_weight(self):
        ...
        npt.assert_equal(self.raster_flow_graph.receivers_weight()[:, 1:], np.zeros((16, 7)))
    ...
    def test_donors_count(self):
        npt.assert_equal(self.profile_flow_graph.donors_count(), np.r_[0, 1, 3, 1, 0, 0, 1, 2])

        npt.assert_equal(self.raster_flow_graph.donors_count(), np.r_[0, 1, 1, 0,
                                                                      0, 0, 0, 5,
                                                                      0, 8, 0, 0,
                                                                      0, 0, 1, 0])

    def test_dfs_stack(self):
        npt.assert_equal(self.profile_flow_graph.dfs_stack(),
                         np.r_[2, 1, 0, 3, 4, 7, 6, 5])

        npt.assert_equal(self.raster_flow_graph.dfs_stack(), 
...
class TestMultipleFlowRouter():
    ...
    def test___init__(self): 
        multiple_router = MultipleFlowRouter(1., 1.5)


        with pytest.raises(TypeError):
            MultipleFlowRouter(1.)

        with pytest.raises(TypeError):
            MultipleFlowRouter(1., "a")

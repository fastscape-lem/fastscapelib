...
void add_flow_routers_bindings(py::module& m) {
    ...
    py::enum_<fs::flow_router_methods> methods(m, "FlowRouterMethods", py::arithmetic(), 
    ...
    py::class_<dummy_flow_router_method, flow_router_method, std::shared_ptr<dummy_flow_router_method>>(m, "DummyFlowRouter")
        .def(py::init());

    // ==== Binding of the SingleFlowRouter class ==== //
    py::class_<single_flow_router_method, flow_router_method, std::shared_ptr<single_flow_router_method>>(m, "SingleFlowRouter")
        .def(py::init());

    // ==== Binding of the MultipleFlowRouter class ==== //
    py::class_<multiple_flow_router_method, flow_router_method, std::shared_ptr<multiple_flow_router_method>>(m, "MultipleFlowRouter")

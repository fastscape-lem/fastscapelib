...
add_flow_graph_bindings(py::module& m)
{
    ...
    py::class_<flow_graph_facade>(m, "FlowGraph")
        .def(py::init<fs::profile_grid&,
                      std::shared_ptr<flow_router_method>,
                      std::shared_ptr<sink_resolver_method>>(),
             py::arg("grid"),
             py::arg("flow_router"),
             py::arg("sink_resolver") = std::make_shared<no_sink_resolver_method>())
        ...
        .def("accumulate",
             py::overload_cast<const xt::pyarray<double, xt::layout_type::row_major>&>(
                 &flow_graph_facade::accumulate, py::const_))
        .def("accumulate",
             py::overload_cast<const double&>(&flow_graph_facade::accumulate, py::const_));
}

...
namespace fastscapelib
{
    ...
    template <class Erosion_XT,
              class Elevation_XT,
              class Stack_XT,
              class Receivers_XT,
              class Dist2Receivers_XT,
              class Area_XT,
              class Float_T>
    void erode_spower(Erosion_XT& erosion,
                      const Elevation_XT& elevation,
                      const Stack_XT& stack,
                      const Receivers_XT& receivers,
                      const Dist2Receivers_XT& dist2receivers,
                      const Area_XT& area,
                      Float_T k,
                      Float_T m,
                      Float_T n,
                      ...
                      Float_T tolerance)
    {
        using Scalar_T = std::common_type_t<Float_T,
                                            typename Erosion_XT::value_type,
                                            typename Elevation_XT::value_type,
                                            typename Area_XT::value_type>;
        ...
        for (const auto istack : stack)
        {
            ...
            const auto factor
                = k * dt * std::pow(area(istack), m) / std::pow(dist2receivers(istack), n);

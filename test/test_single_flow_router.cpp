    ...
    namespace testing
    {
       ...
       class single_flow_router__profile: public ::testing::Test
        ...
        protected:
            ...
            xt::xtensor<double, 1> elevation
                {0.82,  0.16,  0.14,  0.20, 0.71,  0.97,  0.41,  0.09};

            flow_graph_type fixed_graph = flow_graph_type(fixed_grid,
                                        std::make_unique<fs::single_flow_router<flow_graph_type>>(),
                                        std::make_unique<fs::no_sink_resolver<flow_graph_type>>());

            flow_graph_type looped_graph = flow_graph_type(looped_grid,
                                         std::make_unique<fs::single_flow_router<flow_graph_type>>(),
                                         std::make_unique<fs::no_sink_resolver<flow_graph_type>>());
        ...
        TEST_F(single_flow_router__profile, receivers)
        {
            ...
            xt::xtensor<size_type, 1> expected_fixed_receivers
                { 1, 2, 2, 2, 3, 6, 7, 7};
            EXPECT_TRUE(xt::all(xt::equal(xt::col(fixed_graph.receivers(), 0),
            ...
            xt::xtensor<size_type, 1> expected_looped_receivers
                { 7, 2, 2, 2, 3, 6, 7, 7};
            EXPECT_TRUE(xt::all(xt::equal(xt::col(looped_graph.receivers(), 0),
                                          expected_looped_receivers)));
        ...
        TEST_F(single_flow_router__profile, receivers_distance)
        {
            ...
            xt::xtensor<double, 1> expected_looped_receivers_distance
                {1., 1., 0., 1., 1., 1., 1., 0.};
        ...
        TEST_F(single_flow_router__profile, receivers_count)
        {
            ...
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.receivers_count(),
                                          xt::ones<std::uint8_t>({8}))));

            EXPECT_TRUE(xt::all(xt::equal(looped_graph.receivers_count(),
                                          xt::ones<std::uint8_t>({8}))));
        }
        ...
        TEST_F(single_flow_router__profile, receivers_weight)
        {
            ...
            EXPECT_TRUE(xt::all(xt::equal(xt::col(fixed_graph.receivers_weight(), 0),
                                          xt::ones<double>({8}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::col(fixed_graph.receivers_weight(), 1),
                                          xt::zeros<double>({8}))));

            EXPECT_TRUE(xt::all(xt::equal(xt::col(looped_graph.receivers_weight(), 0),
                                          xt::ones<double>({8}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::col(looped_graph.receivers_weight(), 1),
                                          xt::zeros<double>({8}))));
        }
        ...
        TEST_F(single_flow_router__profile, donors)
        {
            ...
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.donors(),
                                          expected_fixed_donors)));
            ...
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.donors(),
                                          expected_looped_donors)));
        }
        ...
        TEST_F(single_flow_router__profile, donors_count)
        {
            ...
            xt::xtensor<std::uint8_t, 1> expected_fixed_donors_count
                {0, 1, 3, 1, 0, 0, 1, 2};
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.donors_count(),
                                          expected_fixed_donors_count)));

            xt::xtensor<std::uint8_t, 1> expected_looped_donors_count
                {0, 0, 3, 1, 0, 0, 1, 3};
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.donors_count(),
                                          expected_looped_donors_count)));
        }
        ...
        TEST_F(single_flow_router__profile, dfs_stack)
        {
            ...
            xt::xtensor<std::uint8_t, 1> expected_fixed_stack
                {2, 1, 0, 3, 4, 7, 6, 5};
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.dfs_stack(),
                                          expected_fixed_stack)));

            xt::xtensor<std::uint8_t, 1> expected_looped_stack
                {2, 1, 3, 4, 7, 0, 6, 5};
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.dfs_stack(),
                                          expected_looped_stack)));
        }
       ...
       class single_flow_router__raster_queen: public single_flow_router__raster_base
        ...
        protected:
            ...
            grid_type fixed_grid = grid_type(shape, {1.1, 1.2}, fs::node_status::fixed_value_boundary);
            grid_type looped_grid = grid_type(shape, {1.1, 1.2}, fs::node_status::looped_boundary);
       ...
       TEST_F(single_flow_router__raster_queen, receivers_distance)
        {
            ...
            EXPECT_TRUE(xt::allclose(xt::col(fixed_graph.receivers_distance(), 0),
                                     expected_fixed_receivers_distance));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(fixed_graph.receivers_distance(), 0, xt::range(1, 8)),
            ...
            EXPECT_TRUE(xt::allclose(xt::col(looped_graph.receivers_distance(), 0),
                                     expected_looped_receivers_distance));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(looped_graph.receivers_distance(), 0, xt::range(1, 8)),
                                          xt::ones<double>({16, 7}) * -1)));
        }
       ...
       TEST_F(single_flow_router__raster_queen, receivers_count)
        ...
        TEST_F(single_flow_router__raster_queen, receivers_weight)
        {
            ...
            EXPECT_TRUE(xt::all(xt::equal(xt::col(fixed_graph.receivers_weight(), 0),
                                          xt::ones<double>({16}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(fixed_graph.receivers_weight(), xt::all(), xt::range(1, 8)),
                                          xt::zeros<double>({16, 7}))));

            EXPECT_TRUE(xt::all(xt::equal(xt::col(looped_graph.receivers_weight(), 0),
                                          xt::ones<double>({16}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(looped_graph.receivers_weight(), xt::all(), xt::range(1, 8)),
                                          xt::zeros<double>({16, 7}))));
        }
        ...
        TEST_F(single_flow_router__raster_queen, donors)
        {
            ...
            xt::view(expected_fixed_donors, 7, xt::range(0, 5)) = xt::xarray<size_type>({2, 3, 6, 7, 11});
            xt::view(expected_fixed_donors, 9, xt::range(0, 8)) = xt::xarray<size_type>({4, 5, 8, 9, 10, 12, 13, 14});
            ...
            xt::view(expected_looped_donors, 7, xt::range(0, 5)) = xt::xarray<size_type>({3, 4, 6, 7, 11});
            xt::view(expected_looped_donors, 9, xt::range(0, 7)) = xt::xarray<size_type>({5, 8, 9, 10, 12, 13, 14});
            xt::view(expected_looped_donors, 14, xt::range(0, 3)) = xt::xarray<size_type>({1, 2, 15});
       ...
       TEST_F(single_flow_router__raster_queen, donors_count)
        ...
        TEST_F(single_flow_router__raster_queen, dfs_stack)
        {
            ...
            xt::xtensor<std::uint8_t, 1> expected_fixed_stack
                {7, 2, 1, 0, 3, 6, 11, 9, 4, 5, 8, 10, 12, 13, 14, 15};
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.dfs_stack(),
                                          expected_fixed_stack)));
       
            xt::xtensor<std::uint8_t, 1> expected_looped_stack
                {7, 3, 4, 6, 11, 9, 5, 8, 10, 12, 13, 14, 1, 0, 2, 15};
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.dfs_stack(),
                                          expected_looped_stack)));
        }
       ...
       TEST_F(single_flow_router__raster_rook, receivers_distance)
        {
            ...
            EXPECT_TRUE(xt::allclose(xt::col(fixed_graph.receivers_distance(), 0),
                                     expected_fixed_receivers_distance));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(fixed_graph.receivers_distance(), 0, xt::range(1, 4)),
            ...
            EXPECT_TRUE(xt::allclose(xt::col(looped_graph.receivers_distance(), 0),
                                     expected_looped_receivers_distance));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(looped_graph.receivers_distance(), 0, xt::range(1, 4)),
                                          xt::ones<double>({16, 3}) * -1)));
        }
       ...
       TEST_F(single_flow_router__raster_rook, receivers_count)
        ...
        TEST_F(single_flow_router__raster_rook, receivers_weight)
        {
            ...
            EXPECT_TRUE(xt::all(xt::equal(xt::col(fixed_graph.receivers_weight(), 0),
                                          xt::ones<double>({16}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(fixed_graph.receivers_weight(), xt::all(), xt::range(1, 4)),
                                          xt::zeros<double>({16, 3}))));

            EXPECT_TRUE(xt::all(xt::equal(xt::col(looped_graph.receivers_weight(), 0),
                                          xt::ones<double>({16}))));
            EXPECT_TRUE(xt::all(xt::equal(xt::view(looped_graph.receivers_weight(), xt::all(), xt::range(1, 4)),
                                          xt::zeros<double>({16, 3}))));
        }
        ...
        TEST_F(single_flow_router__raster_rook, donors)
        {
            ...
            xt::view(expected_fixed_donors, 7, xt::range(0, 3)) = xt::xarray<size_type>({3, 7, 11});
            expected_fixed_donors(8, 0) = 4;
            xt::view(expected_fixed_donors, 9, xt::range(0, 5)) = xt::xarray<size_type>({5, 8, 9, 10, 13});
            expected_fixed_donors(10, 0) = 6;
            expected_fixed_donors(12, 0) = 12;
            xt::view(expected_fixed_donors, 14, xt::range(0, 2)) = xt::xarray<size_type>({14, 15});
            ...
            xt::view(expected_looped_donors, 7, xt::range(0, 4)) = xt::xarray<size_type>({3, 4, 7, 11});
            xt::view(expected_looped_donors, 9, xt::range(0, 5)) = xt::xarray<size_type>({5, 8, 9, 10, 13});
            expected_looped_donors(10, 0) = 6;
            expected_looped_donors(12, 0) = 12;
            xt::view(expected_looped_donors, 14, xt::range(0, 3)) = xt::xarray<size_type>({2, 14, 15});
       ...
       TEST_F(single_flow_router__raster_rook, donors_count)
        ...
        TEST_F(single_flow_router__raster_rook, dfs_stack)
        {
            ...
            xt::xtensor<std::uint8_t, 1> expected_fixed_stack
                {2, 1, 0, 7, 3, 11, 9, 5, 8, 4, 10, 6, 13, 12, 14, 15};
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.dfs_stack(),
                                          expected_fixed_stack)));
       
            xt::xtensor<std::uint8_t, 1> expected_looped_stack
                {7, 3, 4, 11, 9, 5, 8, 10, 6, 13, 12, 14, 2, 1, 0, 15};
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.dfs_stack(),
                                          expected_looped_stack)));
        }
       ...
       TEST_F(single_flow_router__raster_bishop, receivers_count)
        ...
        TEST_F(single_flow_router__raster_bishop, donors)
        {
            ...
            xt::view(expected_fixed_donors, 7, xt::range(0, 3)) = xt::xarray<size_type>({2, 7, 10});
            expected_fixed_donors(8, 0) = 8;
            xt::view(expected_fixed_donors, 9, xt::range(0, 5)) = xt::xarray<size_type>({4, 6, 9, 12, 14});
            xt::view(expected_fixed_donors, 10, xt::range(0, 3)) = xt::xarray<size_type>({5, 13, 15});
            ...
            xt::xarray<size_type> expected_looped_donors = xt::ones<size_type>({16, 5}) * -1;
            xt::view(expected_looped_donors, 7, xt::range(0, 5)) = xt::xarray<size_type>({0, 2, 7, 8, 10});
            xt::view(expected_looped_donors, 9, xt::range(0, 5)) = xt::xarray<size_type>({4, 6, 9, 12, 14});
            xt::view(expected_looped_donors, 10, xt::range(0, 3)) = xt::xarray<size_type>({5, 13, 15});
            xt::view(expected_looped_donors, 14, xt::range(0, 3)) = xt::xarray<size_type>({1, 3, 11});
       ...
       TEST_F(single_flow_router__raster_bishop, donors_count)
        ...
        TEST_F(single_flow_router__raster_bishop, dfs_stack)
        {
            ...
            xt::xtensor<std::uint8_t, 1> expected_fixed_stack
                {0, 1, 3, 7, 2, 10, 5, 13, 15, 8, 9, 4, 6, 12, 14, 11};
            EXPECT_TRUE(xt::all(xt::equal(fixed_graph.dfs_stack(),
                                          expected_fixed_stack)));
       
            xt::xtensor<std::uint8_t, 1> expected_looped_stack
                {7, 0, 2, 8, 10, 5, 13, 15, 9, 4, 6, 12, 14, 1, 3, 11};
            EXPECT_TRUE(xt::all(xt::equal(looped_graph.dfs_stack(),
                                          expected_looped_stack)));
        }

...
TEST(basin_graph, compute_basins_2)
{
    ...
    xt::xtensor<double, 2> elevation {{2.0, 3.0, 3.0, 2.0, 5.0},
                                      {8.0, 0.0, 1.0, 1.0, 4.0},
                                      {2.0, 4.0, 3.0, 2.0, 6.0}};

    xt::xtensor<bool, 2> active_nodes {{false, false, false, false, false},
                                       {false, true,  true,  true, false},
                                       {false, false, false, false, false}};

    xt::xtensor<index_t, 1> receivers = xt::ones<index_t>({15}) * -1;
    xt::xtensor<double, 1> dist2receivers = xt::ones<double>({15}) * -1.;
    xt::xtensor<index_t, 1> ndonors = xt::ones<index_t>({15}) * -1;
    xt::xtensor<index_t, 2> donors = xt::ones<index_t>({15, 8}) * -1;
    xt::xtensor<index_t, 1> stack = xt::ones<index_t>({15}) * -1;
    ...
    //std::cout << t::adapt(outlets, expected_basins.shape()) << std::endl;
...
TEST(basin_graph, connect_basins_basins)
{
    fs::BasinGraph<index_t, index_t> basin_graph;
    ...
    EXPECT_TRUE(basin_graph.is_links_eq(expected_links));
